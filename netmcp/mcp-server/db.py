...
class DynamoDBClient:
    ...
    def _get_recent_sync(self, limit: int) -> List[Any]:
    ...
    def _get_failed_sync(self, limit: int) -> List[Any]:
        result = self.table.scan(
    ...
    async def get_failed_requests(self, limit: int = 20) -> List[Any]:
        return await asyncio.to_thread(self._get_failed_sync, limit)

    def _get_by_url_sync(self, url: str) -> List[Any]:
        result = self.table.scan(
    ...
    async def get_by_url(self, url: str) -> List[Any]:
        return await asyncio.to_thread(self._get_by_url_sync, url)
    ...
    async def search_requests(
        ...
        return await asyncio.to_thread(
            self._search_sync, method, status_code, url_contains, limit
        )

    def _get_slow_sync(self, threshold_ms: int) -> List[Any]:
        result = self.table.scan(
    ...
    async def get_slow_requests(self, threshold_ms: int) -> List[Any]:
        return await asyncio.to_thread(self._get_slow_sync, threshold_ms)
